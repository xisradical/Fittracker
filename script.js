document.addEventListener("DOMContentLoaded", () => {
    ...
    function adicionarItem() {
        ...
        totalRepeticoes += parseFloat(totalPorItem);
        totalGeral.textContent = totalRepeticoes.toFixed(2);

        newRow.innerHTML = `
            <td>${nomeItem}</td>
            <td><input type="number" class="quantidade-editavel" value="${quantidade}" onchange="atualizarCalculo(this)" min="1"></td>
            <td><input type="number" class="preco-editavel" value="${precoUnitario.toFixed(2)}" onchange="atualizarCalculo(this)" step="0.01" min="0.01"></td>
    ...
    function salvarCompra() {
        ...
        headerDiv.innerHTML = `
            Data: ${data} | Total Geral: ${totalRepeticoes.toFixed(2)}
            <br><button onclick="removerHistorico(this)">X</button>
        `;
    ...
    function exportarParaExcel() {
        ...
        const wsItens = XLSX.utils.aoa_to_sheet([
            ["Nome do Exercício", "Repetições", "Número de Séries", "Reps Totais", "Concluído"],
            ...dadosItens
        ]);
        XLSX.utils.book_append_sheet(wb, wsItens, "Itens");
        XLSX.writeFile(wb, "lista_de_exercicios.xlsx");
    }
    ...
    function salvarHistoricoNoLocalStorage() {
        ...
        Array.from(historicoComprasDiv.children).forEach(compraDiv => {
            const headerText = compraDiv.querySelector(".historico-compra-header").textContent;
    ...
    function carregarDadosSalvos() {
        ...
        if (dadosSalvos) {
            ...
            dados.forEach(item => {
                ...
                newRow.innerHTML = `
                    <td>${item.nomeItem}</td>
                    <td><input type="number" class="quantidade-editavel" value="${item.quantidade}" onchange="atualizarCalculo(this)" min="1"></td>
                    <td><input type="number" class="preco-editavel" value="${item.precoUnitario}" onchange="atualizarCalculo(this)" step="0.01" min="0.01"></td>
                    <td class="total-item">${item.totalItem}</td>
                    <td><input type="checkbox" class="promocao-checkbox" ${item.promocao ? "checked" : ""}></td>
                    <td><button onclick="removerItem(this)">X</button></td>
                `;
                totalRepeticoes += parseFloat(item.totalItem);
            });
            totalGeral.textContent = totalRepeticoes.toFixed(2);
        }
